...
class AgendaService:
    ...
    def crear_cita(
        ...
        self._validar_intervalo(start_dt, end_dt)
    ...
    def _validar_intervalo(self, start_dt: datetime, end_dt: datetime) -> None:
        ...
        if (delta.total_seconds() / 60) < config.INTERVALO_MINUTOS:
            raise ValueError("La duración mínima es de 15 minutos")
        if (start_dt.minute % config.INTERVALO_MINUTOS) != 0:
            raise ValueError("La hora de inicio debe alinearse a intervalos de 15 minutos")
